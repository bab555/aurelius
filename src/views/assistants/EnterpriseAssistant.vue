  ...
  <div class="flex-1 flex flex-col relative h-screen">
    ...
    <div class="p-4 md:p-8 flex flex-col h-full">
      ...
      <div class="mb-6 flex-shrink-0">
        <h1 class="text-2xl md:text-3xl font-bold text-white mb-2">企业助手</h1>
        <p class="text-gray-400 text-sm">企业级知识库搭建解决方案试用</p>
      ...
      <div class="flex-1 bg-[#0e0b36] p-4 md:p-6 mb-4 rounded-lg flex flex-col min-h-0">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar min-h-0" ref="chatContainer">
          ...
          <div v-if="messages.length === 0 && !isLoading" class="h-full flex flex-col items-center justify-center text-center px-4">
            ...
            <h3 class="text-lg font-medium mb-2">欢迎使用企业助手</h3>
            <p class="text-gray-400 text-sm max-w-md mb-8">
              我是您的企业知识库管理助手，可以帮助您构建、管理和利用企业专属知识，提供安全可靠的智能问答服务。
            </p>
      ...
      <div class="mt-4 flex-shrink-0">
        ...
        <div class="text-xs text-gray-500 mt-2 text-center">企业知识库助手，助力企业数字化转型</div>
      </div>
    </div>
  </div>
</template>
...
const suggestions = [
  "介绍一下企业知识库的主要功能和应用场景",
  "如何将企业内部文档快速集成到AI助手系统中？",
  "构建企业专属AI助手需要考虑哪些安全因素？",
  "企业知识库与大模型结合有哪些具体应用案例？"
]
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (chatStore.currentConversationId) {
      sessionStorage.setItem('enterprise_conversation_id', chatStore.currentConversationId)
...
onMounted(async () => {
  try {
    // 初始化聊天状态，使用企业助手APP_TYPE
    await chatStore.initialize(chatAPI.APP_TYPES.ENTERPRISE);
    ...
    const sessionId = sessionStorage.getItem('enterprise_conversation_id');
    ...
    if (sessionId) {
      ...
      if (uuidPattern.test(sessionId)) {
        ...
        sessionStorage.removeItem('enterprise_conversation_id');
      }
    }
    ...
    if (!sessionLoaded) {
      ...
      // 设置应用类型为企业助手
      chatStore.setAppType(chatAPI.APP_TYPES.ENTERPRISE);
...
const handleSessionReplaced = (event) => {
  ...
  if (chatStore.currentConversationId === oldSessionId) {
    ...
    messages.value.push({
      ...
      conversation_id: chatStore.currentConversationId,
      role: 'assistant',
      content: '⚠️ 检测到您已在其他标签页中打开了企业助手。为避免冲突，本页面将不再响应。',
      created_at: Date.now() / 1000,
      isError: true
    });
...
const handleVisibilityChange = async () => {
  if (document.visibilityState === 'visible') {
    // 页面变为可见时
    console.log('企业助手页面恢复可见');
    
    // 如果有当前会话ID，检查会话是否仍然有效
    if (chatStore.currentConversationId) {
      try {
        // 检查会话是否存在于sessionStorage中
        const storedId = sessionStorage.getItem('enterprise_conversation_id');

  ...
  <div class="flex-1 flex flex-col relative h-screen">
    ...
    <div class="p-4 md:p-8 flex flex-col h-full">
      ...
      <div class="mb-6 flex-shrink-0">
        <h1 class="text-2xl md:text-3xl font-bold text-white mb-2">智能助手</h1>
        <p class="text-gray-400 text-sm">集成GLM4、Deepseek-r1、千问、混元等多款顶尖大模型，既简化对话交流流程，又实现了多模态图像处理能力，还能在关键时刻进行深度思考分析（注：本助手不支持联网查询，如需联网功能请使用搜索助手）</p>
      ...
      <div class="flex-1 bg-[#0e0b36] p-4 md:p-6 mb-4 rounded-lg flex flex-col min-h-0">
        ...
        >
          ...
          <div v-if="messages.length === 0 && !isLoading" class="h-full flex flex-col items-center justify-center text-center px-4">
            ...
            <h3 class="text-lg font-medium mb-2">欢迎使用智能助手</h3>
            <p class="text-gray-400 text-sm max-w-md mb-8">
              我集成了GLM4、Deepseek-r1、千问、混元等多款顶尖大模型，能高效处理文字与图像内容，解答问题、撰写文档、提供专业分析，让您的工作和学习事半功倍。
            </p>
      ...
      <div class="mt-4 flex-shrink-0">
        ...
        <div class="text-xs text-gray-500 mt-2 text-center">多款顶尖大模型联合协作，解决您的各类问题</div>
      </div>
    </div>
  </div>
</template>
...
// 对话建议
const suggestions = [
  "你能帮我分析一下当前人工智能的发展趋势吗？",
  "写一篇关于数字化转型对传统企业影响的分析报告",
  "帮我设计一个家庭智能家居系统的方案",
...
onMounted(async () => {
  try {
    ...
    const sessionId = sessionStorage.getItem('intelligent_conversation_id');
    ...
    if (!sessionLoaded) {
      ...
      // 设置应用类型为智能助手
      chatStore.setAppType(chatAPI.APP_TYPES.INTELLIGENT);
...
const handleSessionReplaced = (event) => {
  ...
  if (chatStore.currentConversationId === oldSessionId) {
    ...
    messages.value.push({
      ...
      conversation_id: chatStore.currentConversationId,
      role: 'assistant',
      content: '⚠️ 检测到您已在其他标签页中打开了智能助手。为避免冲突，本页面将不再响应。',
      created_at: Date.now() / 1000,
      isError: true
    });
